  ...
  <div class="relative w-full max-w-[400px]" ref="wrapperRef">
    ...
    <div class="relative">
      ...
      <input
        ...
        class="w-full pl-9 pr-8 py-2 border border-gray-300 rounded-md text-sm bg-white text-black focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
      />
      <button
        v-if="searchQuery"
        @click="handleClear"
    ...
    <!-- Loading indicator -->
    <div v-if="isSearching && searchQuery" class="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-md shadow-lg p-3 text-sm text-gray-500 z-50">
      Searching...
    </div>
    ...
    <div
      ...
      class="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-md shadow-lg max-h-[480px] overflow-y-auto z-50"
    >
      <!-- No results message -->
      ...
      <template v-if="searchResults.recipes.length > 0">
        <p class="px-4 pt-3 pb-1 text-xs font-semibold uppercase tracking-wider text-gray-500">Recipes</p>
        <button
        ...
        >
          <i class="mdi mdi-food text-blue-600"></i>
          <div>
      ...
      <template v-if="searchResults.meals.length > 0">
        <hr v-if="searchResults.recipes.length > 0" class="border-gray-200" />
        <p class="px-4 pt-3 pb-1 text-xs font-semibold uppercase tracking-wider text-gray-500">Meals</p>
        <button
          ...
          class="w-full text-left px-4 py-2 hover:bg-gray-50 flex items-center gap-3 transition-colors"
        >
          <i class="mdi mdi-silverware text-green-600"></i>
          <div>
      ...
      <template v-if="searchResults.food_items.length > 0">
        <hr v-if="searchResults.recipes.length > 0 || searchResults.meals.length > 0" class="border-gray-200" />
        <p class="px-4 pt-3 pb-1 text-xs font-semibold uppercase tracking-wider text-gray-500">Food Items</p>
        <button
      ...
      <template v-if="searchResults.ingredients.length > 0">
        <hr v-if="getTotalResults() > searchResults.ingredients.length" class="border-gray-200" />
        <p class="px-4 pt-3 pb-1 text-xs font-semibold uppercase tracking-wider text-gray-500">Ingredients</p>
        <button
          ...
          class="w-full text-left px-4 py-2 hover:bg-gray-50 flex items-center gap-3 transition-colors"
        >
          <i class="mdi mdi-nutrition text-cyan-600"></i>
          <div>
